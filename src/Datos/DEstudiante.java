...
public class DEstudiante {
    ...
    public ArrayList<Estudiante> listarEstudiantes() {
        ...
        try {
            ...
            con.desconectar();
            while (set.next()) {
                lista.add(new Estudiante(set.getInt(1),
                        set.getString(2),
                        set.getString(3),
                        set.getString(4),
                        set.getString(5)
    ...
    public Message registrar(String nombre, String email, String telefono, String password,String tipo) {
        ArrayList<Estudiante> existentUsers = listarEstudiantes();
        boolean nuevoUsr = true;
        for (Estudiante usr : existentUsers) {
        ...
        String query = "INSERT INTO estudiante(nombre, email, telefono, password,tipo)" + "values(?,?,?,?,?)";
        try {
        PreparedStatement ps = con.conectar().prepareStatement(query);
